...
use PhalApi\Api;
use App\Domain\Music as Domain_Music;
/**
 * 音乐
 *
...
class Music extends Api {
    ...
    public function music_list(){
        ...
        $classify=\App\checkNull($this->classify);
    ...
    public function searchMusic(){
        ...
        $rs = array('code' => 0, 'msg' => '', 'info' =>array());
        $keywords=\App\checkNull($this->key);
        $uid=\App\checkNull($this->uid);
        $p=\App\checkNull($this->p);
    ...
    public function collectMusic(){
       ...
       if($checkToken==700){
        ...
        if($res==1001){
            $rs['code']=1001;
            $rs['msg']=\PhalApi\T('该音乐已下架');
            return $rs;
        }

        if($res==200){
            $rs['msg']=\PhalApi\T("取消收藏成功");
        ...
        if($res==201){
            $rs['code']=1002;
            $rs['msg']=\PhalApi\T("取消收藏失败");
            return $rs;
        }

        if($res==300){
            $rs['msg']=\PhalApi\T("收藏成功");
