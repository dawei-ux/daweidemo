...
function hMSet($key,$fieldArr,$time=0){
	...
	$GLOBALS['redisdb']->hmset($key,$fieldArr);
	if($time > 0){
        $GLOBALS['redisdb']->expire($key, $time); 
    }
...
function hIncrBy( $key, $hashKey, $value){
    return $GLOBALS['redisdb']->hIncrBy( $key, $hashKey, $value);
}
 ...
 */
function lSize($key){
    return $GLOBALS['redisdb']->lLen($key);
}
 ...
 */
function exists( $key) {
    return $GLOBALS['redisdb']->exists($key);
}
 ...
 */
function hVals($key) {
    return $GLOBALS['redisdb']->hVals($key);
}
...
function sIsMember($key,$member){
    return $GLOBALS['redisdb']->sIsMember( $key, $member );
}

function sAdd($key,$member){
    return $GLOBALS['redisdb']->sAdd( $key, $member );
}
