...
//		file_put_contents(API_ROOT.'/../log/phalapi/function_sendCodeByTencentSms_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 提交参数信息 result:'.json_encode($result)."\r\n",FILE_APPEND);
	...
	function PrivateKey_sw($host,$stream,$type){
		...
		if($type==1){
			...
			if($host=='rtmp'){
				...
				$url="http://{$pull_url}/live/{$streamKey}.".$ext;
			}
   ...
   /* 扣费 */
	...
	//生成声网回放
	function shengwangPlaybackCurl($url,$bodyArray){
		//官方文档链接：https://doc.shengwang.cn/api-ref/cloud-recording/restful/cloud-recording/operations/post-v1-apps-appid-cloud_recording-acquire
	...
	function getShengWangPlayback($uid,$stream){
		$configpri=\App\getConfigPri();
		$cdn_switch=$configpri['cdn_switch']; //1腾讯2声网
		$sw_playback_tx_switch=$configpri['sw_playback_tx_switch'];  //回放开关
		if($cdn_switch!=2 || !$sw_playback_tx_switch || $configpri['sw_playback_tx_bucket']==''){
			return 0;
		}
		...
		$token=\App\getShengWangRtcToken($liveuid,$stream);
		
		//Acquire：获取云端录制资源
		$acquire_url="https://api.sd-rtn.com/v1/apps/{$appid}/cloud_recording/acquire";
		$acquire_body=[
			'cname' => $stream,
			'uid' => $liveuid,
			'clientRequest' => [
		...
		$acquire_info=\App\shengwangPlaybackCurl($acquire_url,$acquire_body);
		$acquire_info_arr=json_decode($acquire_info, true);
		if(!array_key_exists('resourceId', $acquire_info_arr)){
			return 0;
		}
		//录制的频道名
		$cname=$acquire_info_arr['cname'];
		//云端录制资源 Resource ID。使用这个 Resource ID 可以开始一段云端录制。这个 Resource ID 的有效期为 5 分钟，超时需要重新请求。
		$resourceId=$acquire_info_arr['resourceId'];
		
		//Start：开始云端录制
		//录制模式：individual：单流录制模式。mix：合流录制模式。web：页面录制模式。
		$mode="mix";
		$start_url="https://api.sd-rtn.com/v1/apps/{$appid}/cloud_recording/resourceid/{$resourceId}/mode/{$mode}/start";
		$start_body=[
			'cname' => $stream,
			'uid' => $liveuid,
			'clientRequest' => [
				...
				'storageConfig' => [
						'vendor' => 3,
						'region' => 2,
						'bucket' => $configpri['sw_playback_tx_bucket'],
						'accessKey' => $configpri['sw_playback_tx_accessKey'],
						'secretKey' => $configpri['sw_playback_tx_secretKey'],
						'fileNamePrefix' => [
							'shengwang',
							'playback'
						]
		...
		$start_info=\App\shengwangPlaybackCurl($start_url,$start_body);
		$start_info_arr=json_decode($start_info, true);
		if(!array_key_exists('sid', $start_info_arr)){
			return 0;
		}
		
		//录制 ID
		$sid=$start_info_arr['sid'];  


		//存储-回放生成的 云端录制资源 Resource ID 和 录制 ID
		$key=$stream."_shengwang_playback";
		\App\setcaches($key,['resourceid'=>$resourceId,'sid'=>$sid]);
	}
	
	function setShengWangPlaybackStop($uid,$stream){
		//Stop：停止云端录制
		$configpri=\App\getConfigPri();
		$appid=$configpri['sw_app_id'];
		...
		//存储-回放生成的 云端录制资源 Resource ID 和 录制 ID
		$key=$stream."_shengwang_playback";
		$playback_redis=\App\getcaches($key);
		if(!$playback_redis){
			return 0;
		}
		...
		//云端录制资源 Resource ID
		$resourceId=$playback_redis['resourceid'];
		
		//录制 ID
		$sid_redis=$playback_redis['sid'];
		//录制模式：individual：单流录制模式。mix：合流录制模式。web：页面录制模式。
		$mode="mix";
		
		$stop_url="https://api.sd-rtn.com/v1/apps/{$appid}/cloud_recording/resourceid/{$resourceId}/sid/{$sid_redis}/mode/{$mode}/stop";
		//print_r($stop_url);
		...
		$stop_info=\App\shengwangPlaybackCurl($stop_url,$stop_body);
		$stop_info_arr=json_decode($stop_info, true);
		if(!array_key_exists('serverResponse', $stop_info_arr)){
			return 0;
		}
		
		//当前录制上传的状态："uploaded"：本次录制的文件已经全部上传至指定的第三方云存储。"backuped"：本次录制的文件已经全部上传完成，但是至少有一个 TS 文件上传到了声网备份云。声网服务器会自动将这部分文件继续上传至指定的第三方云存储。"unknown"：未知状态。
		$uploadingStatus=$stop_info_arr['serverResponse']['uploadingStatus'];
		if($uploadingStatus=='unknown'){
			return 0;
		}
		//回放链接
		$fileName=$stop_info_arr['serverResponse']['fileList'][2]['fileName'];
		$video_url=$configpri['sw_playback_tx_url'].$fileName;
		
		//更新回放记录
		\PhalApi\DI()->notorm->live_record
			->where("uid={$uid} and stream='{$stream}'")
