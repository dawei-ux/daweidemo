...
class Cdnrecord extends NotORM {
	/* 直播回放 */
	public function getCdnRecord($id) {
		
		$result=\PhalApi\DI()->notorm->live_record->select('id,starttime,endtime,stream,video_url')->where('id=?',$id)->fetchOne();
...
//        file_put_contents(API_ROOT.'/../log/phalapi/cdnrecord_alicdn_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 提交参数信息 specialParameter:'.json_encode($specialParameter)."\r\n",FILE_APPEND);
...
//        file_put_contents(API_ROOT.'/../log/phalapi/cdnrecord_alicdn_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 返回信息 res_arr:'.json_encode($res_arr)."\r\n\r\n",FILE_APPEND);
        ...
        \PhalApi\DI()->notorm->live_record->where('id=?',$data['id'])->update( array('video_url'=>$video_url) );
