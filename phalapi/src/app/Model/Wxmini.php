...
class Wxmini extends NotORM {

	//获取认证信息
	public function getAuth($uid){
		$auth_info=\PhalApi\DI()->notorm->user_auth
			->where("uid=?",$uid)
			->fetchOne();
		...
		if(!$auth_info){
			...
			$auth_info['front_view']=\App\get_upload_path($auth_info['front_view']);
			$auth_info['back_view']=\App\get_upload_path($auth_info['back_view']);
			$auth_info['handset_view']=\App\get_upload_path($auth_info['handset_view']);
	...
	public function userAuth($data) {
		...
		if($auth_info){
			...
			if($status==2){ //审核失败,重新提交
				...
				$data['addtime']=time();
				$data['reason']='';
				$rs=\PhalApi\DI()->notorm->user_auth->where("uid=?",$uid)->update($data);
				if(!$rs){
				return 1004;
			}
			...
			//写入数据
			$rs=\PhalApi\DI()->notorm->user_auth
			->insert($data);
			if(!$rs){
				return 1004;
			}
	...
	public function profitList($uid,$p){
		...
		$list=\PhalApi\DI()->notorm->cash_record
				->where($where)
				->order("addtime desc")
				->limit(0,$pnum)
				->fetchAll();
	...
	public function goodsOrderRefundConsult($uid,$orderid,$user_type){
        ...
        $orderinfo=\App\getShopOrderInfo($where,"total");
        ...
        //查询退款协商历史
        $refund_list=\App\getShopOrderRefundList(['orderid'=>$orderid]);
	...
	public function getOrderExpressInfo($uid,$orderid,$user_type){
        ...
        $orderinfo=\App\getShopOrderInfo($where,"express_name,express_phone,express_thumb,express_code,express_number,status,province,city,area,address");
        ...
        if($orderinfo['express_thumb']){
            $orderinfo['express_thumb']=\App\get_upload_path($orderinfo['express_thumb']);
        }

        $result=\App\getExpressInfoByKDN($orderinfo['express_code'],$orderinfo['express_number']);


        $result['Success']=\App\is_true($result['Success']);

        if(!$result['Success']){

           $express_status=0;
           $desc=\PhalApi\T('物流查询失败');

        }else{
           $express_status=1;
           $desc=\PhalApi\T('物流查询成功');
        }
    ...
    public function getShopCashRecord($uid,$p){
        ...
        $list=\PhalApi\DI()->notorm->user_balance_cashrecord
                ->where($where)
                ->order("addtime desc")
                ->limit(0,$pnum)
                ->fetchAll();
