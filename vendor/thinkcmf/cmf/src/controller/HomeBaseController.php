...
class HomeBaseController extends BaseController
{
    ...
    protected function initialize()
    {
        ...
        if($module=='portal'||$module=='Portal'||$module=='appapi'||$module=='Appapi'){

            if(isset($_REQUEST['language'])&&$_REQUEST['language']!=''){
                $language=$_REQUEST['language'];
                if(!in_array($language, ['zh-cn','en'])){
                    $language='zh-cn';
                }
            ...
            if($language=='en'){
                $lang_json=[];
                $lang_json1=include(CMF_ROOT.'app/portal/lang/en/common.php');
                $lang_json2=include(CMF_ROOT.'app/appapi/lang/en/common.php');
                $lang_json=array_merge($lang_json1,$lang_json2);

                
            }else{
                $lang_json=[];
                $lang_json1=include(CMF_ROOT.'app/portal/lang/zh-cn/common.php');
                $lang_json2=include(CMF_ROOT.'app/appapi/lang/zh-cn/common.php');
                $lang_json=array_merge($lang_json1,$lang_json2);
            }
        ...
        $this->assign("lang_json",json_encode($lang_json));
        ...
        $key="getLiveClass";
        $live_class = getcaches($key);
        if(!$live_class){
            $live_class = DB::name('live_class')
                ->order('list_order asc,id desc')
                ->select();

            setcaches($key,$live_class);
        }
