...
/* var options = {
    key: fs.readFileSync('/usr/local/ssl/x.com.key'),
    cert: fs.readFileSync('/usr/local/ssl/x.com.crt')
}; */ 
...
io.on('connection', function(socket) {
    ...
    socket.on('conn', function(data) {
        ...
        if(userid<0 && (userid==-9999 || userid=='-9999')){ //app端
            ...
            if(error){
                ...
                if(res != null){
                    ...
                    if(userInfo['id'] == data.uid ){
                        ...
                        //原按照用户等级和送礼物排序
                        //【废弃】socket.sign = Number(userInfo['sign']);
                        ...
                        if( socket.roomnum!=socket.uid && socket.uid >0 ){
                            ...
                            if(userInfo.hasOwnProperty("car")){
                                ...
                                if(carinfo.hasOwnProperty("words_en")){
                                    car_words_en=carinfo['words_en'];
                                }
                            }
                        ...
                        sendSystemMsgLang(socket,"欢迎来到直播间,我们倡导绿色直播。直播内容和封面有违法违规、色情低俗、抽烟喝酒、诱导欺诈、聚众闹事等行为账号会被封禁,网警24小时在线巡查哦！","Welcome to the live broadcast room, we advocate green live broadcasting. Accounts with illegal live content and covers, pornography, smoking and drinking, inducing fraud, gathering crowds to make trouble, etc. will be banned, and the Internet police will patrol online 24 hours a day!");
                        return;
                    }else{
                        socket.disconnect();
                    }
                                                ...
                                                "msg":[
                                                    {
                                                        ...
                                                        "giftname_en":resObj['giftname_en'],
                ...
                "msg":[
                    {
                        ...
                        "ct_en":"You have been involved in a violation, please correct it immediately, otherwise the live room will be closed"
        ...
        // console.log('disconnect');
            ...
            d.run(function() {
                ...
                if(socket.roomnum==socket.uid){
                    /* 主播 */ 
                    if(socket.reusing==0){
                        endLiveConnect(io,socket.uid,function(){

                            // console.log("Live.stopRoom");
                            // console.log(FormatNowDate());
...
function sendSystemMsgLang(socket,msg,msg_en){
    var data_obj={
                    "msg":[
                        {
                            "_method_":"SystemNot",
...
/* 主播连麦结束处理 */
function endLiveConnect(io,uid,fn){
    ...
    var pkuid=0;
    //查询pkuid
    request(config['WEBADDRESS']+"?service=Livepk.getPkUid&uid="+uid,function(error, response, body){
            if(error) return;
            //console.log('changeLive');
            //console.log(body);
            var res = evalJson(body);
            if( response.statusCode == 200 && res.data.code == 0){
                pkuid=res.data.info[0]['pkuid'];
