    ...
    inituser: function () {
        ...
        $.ajax({
        	...
        	success:function(data){
        		console.log(data);
        		if(data.error==0){
					socket.emit('conn', {uid:data.userinfo.id,roomnum: to_uid,stream:room_id,nickname: data.userinfo.user_nickname,equipment: 'pc',token:data.userinfo.token});
                    if(User){
					...
					/* setInterval(function () {
						if (enterChat != 1) {
							...
							//socket.emit('conn', {uid: window.YB_JS_CONF.loginUserInfo.id,roomnum: window._DATA.author.id,nickname: window.YB_JS_CONF.loginUserInfo.user_nickname,equipment: 'pc',token:window.YB_JS_CONF.loginUserInfo.token});
						}
					}, 2000); */							 	 
				}else{
...
var JsInterface={
	...
	chatFromSocket: function(data) {
		...
		}else if(msgmethod=='LiveConnect'){ //主播和主播连麦
	...
	sendMsg:function(data){
        ...
        var _msg = '';
        if(msgtype==0){
			_msg = '<p><label class ="user_nickname" user_id='+data.ct.uid+'><img src="'+level[data.ct.level]['thumb']+'" style="margin-top: -2px;margin-right:2px;" width="35" height="15" >'+data.ct.user_nickname+'</label> : <font color="#ffffff">进入房间</font></p>'
        }else if(msgtype == 2) {
	...
	liveConnect:function(data){
        ...
        if(_DATA.cdn_switch==2 && (_DATA.live.uid != User.id)){
        	var action=data.action;

        	//主播连麦成功
        	if(action==4){

        		var link_stream=data.tostream;

        		_DATA.live.pkuid=data.pkuid;
        		_DATA.live.pkstream=link_stream;

        		var uid=Number($("#uid").val());

		        $.ajax({
		        	url:'/wxshare/share/getLinkMicSwRtcToken',
		        	type:'post',
		        	data:{'uid':uid,'stream':link_stream},
		        	dataType:'json',
		        	success:function(data){
		        		var code=data.code;
		        		if(code !=0){
		        			layer.msg('请重新进入直播间观看主播连麦');
		        			return;
		        		}
		        		var linkmic_token=data.info[0].token;

		        		try{

		        			var player_area_width = $(window).width()/2;
		        			$("#player-"+_DATA.live.uid).width(player_area_width);
					        ...
					        sw_linkmic_options.channel = link_stream;
					        sw_linkmic_options.uid = uid;
					        sw_linkmic_options.appid = $("#appid").val();
					        sw_linkmic_options.token = linkmic_token;
        	...
        	if(action==5){
        		try{
        			...
        			sw_linkmic_client.leave();
        			$("#player-wrapper-"+_DATA.live.pkuid).remove();
        			_DATA.live.pkuid=0;
        			_DATA.live.pkstream='';

        			var player_area_width = $(window).width();
			        var player_area_height=$(window).height();
		        	$("#player-"+_DATA.live.uid).height(player_area_height);
		        	$("#player-"+_DATA.live.uid).width(player_area_width);
		        	$("#player-wrapper-"+_DATA.live.uid).css('float','none');
