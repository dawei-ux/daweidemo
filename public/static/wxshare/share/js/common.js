...
//    //点击发送信息设置
    ...
    flymsgfn:function(url){
        if(fly=="FlyMsg"){
            ...
            var txt=$("#message").val();
            txt=filter(txt);
			var msg = '{"msg":[{"_method_":"SendMsg","action":0,"ct":"'+txt+'","msgtype":"2","uid":"'+User.id+'","uname":"'+User.user_nickname+'","level":"'+User.level+'","vip_type":"'+ User.vip.type +'","liangname":"'+ User.liang.name+'","usertype":"'+ User.usertype+'","guard_type":"'+ User.guard_type+'"}],"retcode":"000000","retmsg":"OK"}';
		...
		$.ajax({
            ...
            success:function(data){
				...
				if(data.errno==0){
					...
					var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"SendGift","action":"0","ct":"'+data.gifttoken+'","msgtype":"1","roomnum":"'+to_uid+'","livename":"'+data.livename+'","level":"'+data.level+'","uid":"'+data.uid+'","uname":"'+User.user_nickname+'","uhead":"'+User.avatar+'","vip_type":"'+ User.vip.type +'","liangname":"'+ User.liang.name+'","usertype":"'+ User.usertype+'","guard_type":"'+ User.guard_type+'"}]}';
					Socket.emitData('broadcast',msg);
				}else{
					_this.countdown_hide();
