	...
	function apply_judge(){
		...
		if(hasEmoji(name)){
			layer.msg(LangT("家族名称不能含有表情"));
			return !1;
		}
		if(hasEmoji(fullname)){
			layer.msg(LangT("真实姓名不能含有表情"));
			return !1;
		}
		if(hasEmoji(carded)){
			layer.msg(LangT("证件号码不能含有表情"));
			return !1;
		}
			...
			success: function(data)
			{
				if(data.state==0){
					...
					layer.alert('你的家族'+data['name']+'申请成功', 
					{
						...
						window.location.href="/appapi/Family/home?uid="+data['uid']+"&token="+data['token']+"&language="+language_type;
					});
				}
			}
		});
	...
	function signout_post(_this,touid,type,reason){
		...
		$.ajax({
			...
			type:'POST',
			data:{uid:uid,token:token,familyid:familyid,touid:touid,type:type,reason:reason,language:language_type},
			dataType:'json',
			success:function(data){
				isbuy=1;
				if(data.code==0){
...
$(function(){ 
	...
	$(".attended .search_btn").on("click",function(){
		...
		$.ajax({
			...
			type:'POST',
			data:{uid:uid,token:token,key:key,language:language_type},
			dataType:'json',
			success:function(data){
				isbuy=1;
				if(data.code==0){
					...
					for(var i=0;i<length;i++){
						html+='<li>';
						html+='<a href="/appapi/Family/detail?familyid='+list[i]['id']+'&uid='+uid+'&token='+token+'&language='+language_type+'">';
						...
						html+='	<div class="info">\
									...
									<p class="info-des"><span>'+LangT('成员')+'：'+list[i]['count']+LangT('人')+'</span><span>ID：'+list[i]['id']+'</span></p>\
								</div>';
						...
						// }else{
							html+='<span class="ok" data-familyid="'+list[i]['id']+'">'+LangT('加入')+'</span>';
						// }
					...
					if(length==0){
						//layer.msg('暂无相关家族');
                        html='<div class="family_empty">'+LangT('你搜索的家族不存在')+'</div>';
                        $(".attended .user-list ul").html(html);
					}
				}else{
					layer.msg(data.msg);
	...
	$(".attended .list_title .reload").on("click",function(){
		...
		$.ajax({
			...
			type:'POST',
			data:{uid:uid,language:language_type},
			dataType:'json',
			success:function(data){
				isbuy=1;
				if(data.code==0){
	...
	$(".apply_wait .revoke").on("click",function(){
		...
		$.ajax({
			...
			type:'POST',
			data:{uid:uid,token:token,language:language_type},
			dataType:'json',
			success:function(data){
				isbuy=1;
				if(data.code==0){
	...
	$(".home .edit_divide").on("click",function(){
		...
		layer.prompt({
			...
			$.ajax({
				...
				type:'POST',
				data:{uid:uid,token:token,familyid:familyid,divide:pass,language:language_type},
				dataType:'json',
				success:function(data){
					isbuy=1;
					if(data.code==0){
	...
	$(".setdes .setdes_post").on("click",function(){
		...
		$.ajax({
			...
			type:'POST',
			data:{uid:uid,token:token,familyid:familyid,des:des,language:language_type},
			dataType:'json',
			success:function(data){
				isbuy=1;
				if(data.code==0){
	...
	$(".member .user-list ul li .action span.ok").on("click",function(){
		...
		layer.prompt(
			...
			if(pass<0||pass>100){
				layer.msg(LangT("分成比例必须是0-100之间的整数"));
				isbuy=1;
				return !1;
			}
			...
			$.ajax({
				...
				type:'POST',
				data:{uid:uid,token:token,familyid:familyid,touid:touid,divide:pass,language:language_type},
				dataType:'json',
				success:function(data){
					isbuy=1;
					if(data.code==0){
	...
	$(".member .user-list ul li .action span.no").on("click",function(){
		...
		layer.confirm(LangT('要把成员')+name+LangT('踢出家族吗？'),{
            title:LangT('提示'),
			...
			layer.prompt(
				...
				$.ajax({
					...
					type:'POST',
					data:{uid:uid,token:token,familyid:familyid,touid:touid,reason:pass,language:language_type},
					dataType:'json',
					success:function(data){
						isbuy=1;
						if(data.code==0){
	...
	$(".detail .sign").on("click",function(){
		...
		$.ajax({
			...
			type:'POST',
			data:{uid:uid,token:token,familyid:familyid,language:language_type},
			dataType:'json',
			success:function(data){
				isbuy=1;
				if(data.code==0){
					layer.msg(data.msg,{},function(){
						window.location.href="/appapi/Family/home?uid="+uid+"&token="+token+"&language="+language_type;
					});
				}else{
					layer.msg(data.msg);
					return !1;
